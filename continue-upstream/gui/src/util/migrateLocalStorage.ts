...
function normalizeLuminaToolPolicies(dispatch: AppDispatch) {
  const migrationKey = "luminaCode.toolPolicies.autoExecute.v1";
  ...
  if (persistedRedux) {
    ...
    if (uiState) {
      const parsedSettings = JSON.parse(uiState)?.toolSettings;
      if (parsedSettings) {
        Object.entries(parsedSettings).forEach(([toolName, policy]) => {
...
export function migrateLocalStorage(dispatch: AppDispatch) {
  ...
  normalizeLuminaToolPolicies(dispatch);
}
