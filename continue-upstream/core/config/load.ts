...
import { loadMarkdownSkills } from "./markdown/loadMarkdownSkills.js";
...
async function finalToBrowserConfig(
  ...
  // Surface skills (procedural memory) to the GUI so the Skills settings section
  // stays aligned with what read_skill / create_skill actually see on disk.
  let skills: Skill[] = [];
  try {
    skills = (await loadMarkdownSkills(ide)).skills;
  } catch {
    skills = [];
  }
  return {
