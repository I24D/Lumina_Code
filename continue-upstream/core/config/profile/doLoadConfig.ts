...
import {
  armOnDemandIdleShutdown,
  isOnDemandMcpServer,
} from "../../context/mcp/mcpIdleManager";
...
export default async function doLoadConfig(options: {
  ...
  for (const server of mcpServerStatuses) {
    ...
    // On-demand MCP servers (e.g. Lumina CAI) may be idle-disconnected to save
    // resources. Keep advertising their cached tools so the model can still
    // call them; the call transparently reconnects the backend (see
    // mcpIdleManager). Arm the idle timer while they are connected.
    const onDemand = isOnDemandMcpServer(server.id);
    if (onDemand && server.status === "connected") {
      armOnDemandIdleShutdown(server.id);
    }
    const includeMcpTools =
      server.status === "connected" ||
      (onDemand && server.tools.length > 0);
    if (includeMcpTools) {
