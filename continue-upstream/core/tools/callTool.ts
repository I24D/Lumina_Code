...
import { ensureOnDemandMcpReady } from "../context/mcp/mcpIdleManager";
...
import { luminaCreateSkillImpl } from "./implementations/luminaCreateSkill";
...
import { luminaWhatsAppImpl } from "./implementations/luminaWhatsApp";
import { luminaPhoneLinkImpl } from "./implementations/luminaPhoneLink";
import { luminaRuntimeImpl } from "./implementations/luminaRuntime";
...
async function callToolFromUri(
  ...
  switch (parsedUri?.protocol) {
    ...
    case "mcp:":
      ...
      // On-demand servers (e.g. Lumina CAI) may be idle-disconnected: spin the
      // backend back up before calling, and reset its idle-shutdown timer.
      await ensureOnDemandMcpReady(mcpId);
...
export async function callBuiltInTool(
  ...
  switch (functionName) {
    ...
    case BuiltInToolNames.LuminaWindowsBridge:
      return await luminaWindowsBridgeImpl(args, extras);
    case BuiltInToolNames.LuminaWhatsApp:
      return await luminaWhatsAppImpl(args, extras);
    case BuiltInToolNames.LuminaPhoneLink:
      return await luminaPhoneLinkImpl(args, extras);
    case BuiltInToolNames.LuminaRuntime:
      return await luminaRuntimeImpl(args, extras);
    case BuiltInToolNames.GenerateImage:
      return await luminaImageGenImpl(args, extras);
    case BuiltInToolNames.GenerateVideo:
      return await luminaVideoGenImpl(args, extras);
    case BuiltInToolNames.CreateSkill:
      return await luminaCreateSkillImpl(args, extras);
    case BuiltInToolNames.ManageSkills:
      return await manageSkillsImpl(args, extras);
    case BuiltInToolNames.ManageTodos:
