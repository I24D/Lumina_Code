...
describe("runTerminalCommandImpl", () => {
  ...
  it("should strip accidental outer quotes before executing", async () => {
    const command = `'node -e "console.log('quoted command stdout')"'`;
    const result = await runTerminalCommandImpl(
      { command, waitForCompletion: true },
      createMockExtras(),
    );

    expect(result[0].content).toContain("quoted command stdout");
    expect(result[0].content).not.toContain("node -e");
    expect(result[0].status).toBe("Command completed");
  });
  ...
  describe("cwd handling", () => {
    ...
    describe("remote environment handling", () => {
      ...
      it("should capture output when VS Code returns no remoteName", async () => {
        mockGetIdeInfo.mockResolvedValue({ remoteName: undefined });

        const result = await runTerminalCommandImpl(
          {
            command: `node -e "console.log('stdout captured without remote name')"`,
            waitForCompletion: true,
          },
          createMockExtras(),
        );
        ...
        expect(result[0].content).toContain(
          "stdout captured without remote name",
        );
      });
