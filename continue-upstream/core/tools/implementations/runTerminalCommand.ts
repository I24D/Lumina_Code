...
function resolveWorkingDirectory(workspaceDirs: string[]): string {
  ...
  const fileWorkspaceDirs = workspaceDirs.filter((dir) =>
    dir.startsWith("file:/"),
  );
  for (const fileWorkspaceDir of fileWorkspaceDirs) {
    try {
      const candidate = fileURLToPath(fileWorkspaceDir);
  ...
  try {
    ...
    return isUsableDirectory(fallback) ? fallback : os.tmpdir();
  } catch {
...
function stripAccidentalOuterQuotes(command: string): string {
  ...
  const firstToken = inner.split(/\s+/u)[0]?.toLowerCase() ?? "";
  const commandLike =
    firstToken.includes(":\\") ||
    firstToken.includes("/") ||
    /^[\w.-]+(?:\.exe|\.cmd|\.bat|\.ps1)?$/iu.test(firstToken);

  return commandLike ? inner : command;
}
...
function isLocalIde(remoteName: string | null | undefined): boolean {
...
export const runTerminalCommandImpl: ToolImpl = async (args, extras) => {
  const command = stripAccidentalOuterQuotes(getStringArg(args, "command"));
  ...
  if (isLocalIde(ideInfo.remoteName)) {
