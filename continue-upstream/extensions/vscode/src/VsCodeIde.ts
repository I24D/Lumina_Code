    ...
    return new Promise<string>((resolve, reject) => {
      ...
      p.on("close", (code) => {
        if (code === 0) {
          ...
          // No matches
          resolve(
            "No matches found. Build/cache dirs and files (and paths in .gitignore/.continueignore) are not included.",
          );
        } else {
    ...
    if (vscode.env.remoteName) {
      ...
      for (const dir of await this.getWorkspaceDirs()) {
        const dirResults = await this.runRipgrepQuery(dir, [
          ...
          "--ignore-file",
          ".continueignore",
          // Lumina Code: do NOT hide files listed in .gitignore (e.g. `.env`)
          // from agent search. Behave like full filesystem access; only
          // build/cache noise stays filtered via the glob below.
          "--no-ignore-vcs",
          "--glob",
          defaultIgnoresGlob,
  ...
  async getSearchResults(query: string, maxResults?: number): Promise<string> {
    ...
    for (const dir of await this.getWorkspaceDirs()) {
      const dirResults = await this.runRipgrepQuery(dir, [
        ...
        "--ignore-file",
        ".continueignore",
        // Lumina Code: do NOT hide .gitignore'd files (e.g. `.env`) from agent
        // grep. Full access; build/cache noise still filtered via the glob.
        "--no-ignore-vcs",
        "-C",
