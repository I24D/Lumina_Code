...
def _build_system_prompt() -> str:
    """Vision prompt, built from config so this second Live session introduces
    itself with the same name and gender as the main one — a hardcoded identity
    here meant the assistant changed persona the moment it looked at a screen."""
    cfg    = _load_config()
    name   = (cfg.get("assistant_name") or "JARVIS").strip()
    gender = (cfg.get("assistant_gender") or "").strip().lower()

    gender_line = ""
    if gender in ("female", "male"):
        form = "feminine" if gender == "female" else "masculine"
        gender_line = (
            f"You are {gender}. In every language that marks gender on adjectives, "
            f"participles or nouns, speak about yourself in the {form} form. "
        )
...
class _VisionSession:
    ...
    async def _session_loop(self) -> None:
        ...
        while True:
            try:
                # Built per connect, not once per process: a voice or name change
                # in the UI rebuilds the main session immediately, and this one
                # has to follow on its next reconnect instead of keeping the
                # identity it happened to launch with.
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript and self._player:
                        ...
                        if full:
                            _name = (_load_config().get("assistant_name")
                                     or "JARVIS").strip()
                            self._player.write_log(f"{_name}: {full}")
                            print(f"[Vision] 💬 {full}")
                    transcript = []
