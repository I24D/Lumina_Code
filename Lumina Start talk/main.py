...
from actions.agent_bridge      import AgentBridge
...
class JarvisLive:
    ...
    def __init__(self, ui: JarvisUI):
        ...
        # Respuestas de Claude Code / Codex / Lumina Code y notificaciones de
        # Windows. Se arranca aquí (no dentro de la sesión) porque el hook de
        # Claude Code puede postear en cualquier momento, también mientras la
        # sesión de voz se está reconectando: lo que llegue se queda en la cola.
        self._agent_bridge     = AgentBridge(log=self.ui.write_log)
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            _gender = (_cfg.get("assistant_gender") or "").strip().lower()
        except Exception:
        ...
        # Gender is part of identity, not decoration: Spanish, French, Turkish and
        # most of the languages this thing is spoken to in mark the speaker's
        # gender on ordinary adjectives. Without this line the model picks one at
        # random per sentence and the assistant misgenders itself mid-conversation.
        _gender_line = ""
        if _gender in ("female", "male"):
            _form = "feminine" if _gender == "female" else "masculine"
            _gender_line = (
                f"You are {_gender}. In every language that marks gender on "
                f"adjectives, participles or nouns, refer to yourself in the "
                f"{_form} form — in every language, not only the one these "
    ...
    # ── Puente de agentes y notificaciones ──────────────────────────────────────

    async def _run_agent_bridge(self) -> None:
        """Habla lo que llega de Claude Code, Codex, Lumina Code y Windows.

        Mismas reglas de cortesía que el monitor de sistema: nunca encima de la
        voz del usuario ni de la suya propia. La diferencia es que aquí NO se
        descarta lo que no se puede decir todavía —una respuesta de agente sigue
        interesando diez segundos después—, así que se espera y se reintenta.
        """
        pending = None
        while True:
            await asyncio.sleep(1.5)
            if pending is None:
                pending = await asyncio.to_thread(self._agent_bridge.poll)
            if pending is None or not self.session:
                continue

            with self._speaking_lock:
                speaking = self._is_speaking
            if speaking or (time.monotonic() - self._last_user_speech) < 6:
                continue          # se conserva `pending` para el siguiente ciclo

            try:
                await self.session.send_client_content(
                    turns={"parts": [{"text": pending.as_prompt()}]},
                    turn_complete=True,
                )
                self.ui.write_log(f"BRIDGE: {pending.source} — anunciado.")
            except Exception as e:
                print(f"[Bridge] ⚠️ Could not announce: {e}")
            finally:
                pending = None
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    tg.create_task(self._run_agent_bridge())
